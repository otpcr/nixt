...
def cor(event):
    ...
    for _fn, email in find("email", {"From": event.args[0]}):
        ...
        if len(event.args) > 1:
            ...
            txt = "From,Subject"
        event.reply("%s %s %s" % (nr, format(email, txt, plain=True), laps(time.time() - fntime(email.__stp__))))
...
def eml(event):
    ...
    for fn, o in find("email"):
        if event.rest in o.text:
            nr += 1
            event.reply("%s %s %s" % (nr, format(o, "From,Subject"), laps(time.time() - fntime(fn))))
